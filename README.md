...
| 27.| Addition of Two Square Matrices         | Easy       | [GFG]([https://www.geeksforgeeks.org/problems/addition-of-two-square-](https://www.geeksforgeeks.org/problems/addition-of-two-square-matrices4916/1))|  ✅ Done | — | Single transaction: keep|
...
matrices4916/1) | ✅ Done|sonu  |Add B into A: matrixA[i][j] += matrixB[i][j] `minPrice` so far; for each `price[i]` update `maxProfit = max(maxProfit, price[i] - minPrice)`; then `minPrice = min(minPrice, price[i]).|
