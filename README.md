...
| 4 | Binary Search | https://www.geeksforgeeks.org/problems/binary-search-1587115620/1?page=1&sprint=63af446b97dfa5601c7b06b52b991d07&sortBy=submissions | Use two pointers (low=0, high=n-1). While low<=high: mid = low + (high-low)/2. If arr[mid]==x return mid, else if arr[mid]<x low=mid+1, else high=mid-1. Works only on a sorted array. Time: O(log n), Space: O(1). |
