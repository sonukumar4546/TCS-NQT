...
| 22 | Check if an Array is Sorted | Easy | [GFG](https://www.geeksforgeeks.org/problems/check-if-an-array-is-sorted0701/1) | ✅ Done | — | — |Add B into A: matrixA[i][j] += matrixB[i][j] `min)|
...
| 26 | Buy Stock 2 | Medium | [GFG](https://www.geeksforgeeks.org/problems/buy-stock-2/1) |  ✅ Done  ||Add B into A: matrixA[i][j] += matrixB[i][j] `min)|
...
| 48 | Insert a node in a BST | Medium | [GFG](https://www.geeksforgeeks.org/problems/insert-a-node-in-a-bst/1) | ✅ Done | — | BST insert: if `root==NULL` return `new Node(key)`; else if `key < root->data` do `root->left = insert(root->left, key)`; else if `key > root->data` do `root->right = insert(root->right, key)`; return `root` (ignore duplicates if required). |
| 49 | Floor in BST | Medium | [GFG](https://www.geeksforgeeks.org/problems/floor-in-bst/1) | ✅ Done | — | Traverse BST: if `node->data == x` return it; if `node->data < x`, store as ans and go right; else go left. |
| 50 | Inorder Traversal | Easy | [GFG](https://www.geeksforgeeks.org/problems/inorder-traversal/1) | ✅ Done | — | Inorder: Left → Node → Right. Use a `void` helper and pass `vector<int>&` to avoid creating a vector inside recursion. |
| 51 | Decimal to Binary | Easy | [GFG](https://www.geeksforgeeks.org/problems/decimal-to-binary-1587115620/1) | ✅ Done | — | Repeatedly take `n%2` (or `n&1`), build bits, then reverse (handle `n==0`). |
