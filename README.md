...
| 26 | Buy Stock 2 | Medium | [GFG](https://www.geeksforgeeks.org/problems/buy-stock-2/1) |   |Add B into A: matrixA[i][j] += matrixB[i][j] `minPrice` so far; for each `price[i]` update `maxProfit = max(maxProfit, price[i] - minPrice)`; then `minPrice = min(minPrice, price[i]).|
